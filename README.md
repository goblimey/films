...
Note that things like table and database names are case-sensitive when MySQL runs under UNIX, so the databases "FILMS", "Films" and "films" are not the same.  Under Windows those names would all apply to the same objects.  (This is because the objects are represented by files, and those are the naming rules for files on UNIX and Windows.)
...
Go also offers a framework that you can use to build a complete set of system tests for a web server, making web requests and checking that the correct response comes back.  You can use that, but of course, you still have to test your server using a web browser, preferably using all the common web browsers, to make sure that everything looks sensible.  I choose to use Selenium for system testing web servers because it's visual, it can record tests interactively and it runs in a real browser.
